...
import xianzhan.pascal.backend.compiler.generator.ProgramGenerator;
...
public class CodeGenerator extends Backend {
    ...
    protected void emitLoadConstant(int value) {
        switch (value) {
            ...
            case 1:
                ...
                break;
            case 2:
                emit(Instruction.ICONST_2);
                break;
            case 3:
                emit(Instruction.ICONST_3);
                break;
            case 4:
                emit(Instruction.ICONST_4);
                break;
            case 5:
                emit(Instruction.ICONST_5);
                break;
    ...
    protected void emitLoadLocal(TypeSpec type, int index) {
        ...
        if ((type == Predefined.integerType) ||
                ...
                (form == TypeFormEnumImpl.ENUMERATION)) {
            switch (index) {
                case 0:
                    emit(Instruction.ILOAD_0);
                    break;
                case 1:
                    emit(Instruction.ILOAD_1);
                    break;
                case 2:
                    emit(Instruction.ILOAD_2);
                    break;
                case 3:
                    emit(Instruction.ILOAD_3);
                    break;
                default:
                    emit(Instruction.ILOAD, index);
            }
        } else if (type == Predefined.realType) {
            switch (index) {
                case 0:
                    emit(Instruction.FLOAD_0);
                    break;
                case 1:
                    emit(Instruction.FLOAD_1);
                    break;
                case 2:
                    emit(Instruction.FLOAD_2);
                    break;
                case 3:
                    emit(Instruction.FLOAD_3);
                    break;
                default:
                    emit(Instruction.FLOAD, index);
            }
        } else {
            switch (index) {
                case 0:
                    ...
                    break;
                case 1:
                    emit(Instruction.ALOAD_1);
                    break;
                case 2:
                    emit(Instruction.ALOAD_2);
                    break;
                case 3:
                    emit(Instruction.ALOAD_3);
                    break;
                default:
                    emit(Instruction.ALOAD, index);
            }
        }
    }
    ...
    protected void emitLoadVariable(SymTabEntry variableId) {
        ...
        // Wrapped variable.
            ...
            int slot = (Integer) variableId.getAttribute(SymTabKeyImpl.SLOT);
            emitLoadLocal(null, slot);
            emit(Instruction.GETFIELD, varParamWrapper(variableType) + "/value", typeDescriptor(variableType));
        }
    ...
    protected void emitLoadArrayElement(TypeSpec elmtType) {
        ...
        // Load an array element.
            ...
            emit(elmtType == Predefined.integerType
                    ? Instruction.IALOAD : elmtType == Predefined.realType
                    ? Instruction.FALOAD : elmtType == Predefined.booleanType
                    ? Instruction.BALOAD : elmtType == Predefined.charType
                    ? Instruction.CALOAD : form == TypeFormEnumImpl.ENUMERATION
                    ? Instruction.IALOAD : Instruction.AALOAD);
        }
    }
    ...
    protected void emitStoreLocal(TypeSpec type, int index) {
        ...
        if ((type == Predefined.integerType) ||
            ...
            switch (index) {
                case 0:
                    emit(Instruction.ISTORE_0);
                    break;
                case 1:
                    emit(Instruction.ISTORE_1);
                    break;
                case 2:
                    emit(Instruction.ISTORE_2);
                    break;
                case 3:
                    emit(Instruction.ISTORE_3);
                    break;
                default:
                    emit(Instruction.ISTORE, index);
            }
        } else if (type == Predefined.realType) {
            switch (index) {
                case 0:
                    emit(Instruction.FSTORE_0);
                    break;
                case 1:
                    emit(Instruction.FSTORE_1);
                    break;
                case 2:
                    emit(Instruction.FSTORE_2);
                    break;
                case 3:
                    emit(Instruction.FSTORE_3);
                    break;
                default:
                    emit(Instruction.FSTORE, index);
            }
        } else {
            switch (index) {
                case 0:
                    emit(Instruction.ASTORE_0);
                    break;
                case 1:
                    emit(Instruction.ASTORE_1);
                    break;
                case 2:
                    emit(Instruction.ASTORE_2);
                    break;
                case 3:
                    emit(Instruction.ASTORE_3);
                    break;
                default:
                    emit(Instruction.ASTORE, index);
            }
        }
    }
    ...
    protected void emitStoreVariable(SymTabEntry variableId, int nestingLevel, int index) {
        ...
        // Wrapped parameter: Set the wrapper's value field.
            ...
            emitRangeCheck(variableType);
            emit(Instruction.PUTFIELD, varParamWrapper(variableType.baseType()) + "/value", typeDescriptor(variableType.baseType()));
        }
    ...
    protected void emitStoreArrayElement(TypeSpec elmtType) {
        ...
        if (elmtType == Predefined.charType) {
            ...
            emit(elmtType == Predefined.integerType
                    ? Instruction.IASTORE : elmtType == Predefined.realType
                    ? Instruction.FASTORE : elmtType == Predefined.booleanType
                    ? Instruction.BASTORE : elmtType == Predefined.charType
                    ? Instruction.CASTORE : form == TypeFormEnumImpl.ENUMERATION
    ...
    protected void emitRangeCheck(TypeSpec targetType) {
        if (targetType.getForm() == TypeFormEnumImpl.SUBRANGE) {
            int min = (Integer) targetType.getAttribute(TypeKeyEnumImpl.SUBRANGE_MIN_VALUE);
            int max = (Integer) targetType.getAttribute(TypeKeyEnumImpl.SUBRANGE_MAX_VALUE);
    ...
    protected boolean isStructured(TypeSpec type) {
        ...
        return type.isPascalString() || (form == TypeFormEnumImpl.ARRAY) || (form == TypeFormEnumImpl.RECORD);
    }
    ...
    protected boolean isWrapped(SymTabEntry variableId) {
        ...
        return (definition == DefinitionEnumImpl.VAR_PARM) && (form != TypeFormEnumImpl.ARRAY) && (form != TypeFormEnumImpl.RECORD);
    }
    ...
    protected boolean needsCloning(SymTabEntry formalId) {
        ...
        TypeForm form = type.getForm();
        Definition definition = formalId.getDefinition();

        // Arrays and records are normally passed by reference
        // and so must be cloned to be passed by value.
        return (definition == DefinitionEnumImpl.VALUE_PARM) && ((form == TypeFormEnumImpl.ARRAY) || (form == TypeFormEnumImpl.RECORD));
    }
    ...
    protected String typeDescriptor(SymTabEntry id) {
        ...
        if (type != null) {
            if (isWrapped(id)) {
                return "L" + varParamWrapper(type.baseType()) + ";";
            } else {
    ...
    protected String typeDescriptor(TypeSpec type) {
        ...
        while ((form == TypeFormEnumImpl.ARRAY) && !type.isPascalString()) {
            buffer.append("[");
    ...
    protected String valueSignature(TypeSpec type) {
        String javaType = javaTypeDescriptor(type);
        String typeCode = typeDescriptor(type);
        String typeName = type == Predefined.integerType
                ? "int" : type == Predefined.realType
                ? "float" : type == Predefined.booleanType
                ? "boolean" : type == Predefined.charType
                ? "char" : "int";
     ...
     */
    protected String varParamWrapper(TypeSpec type) {
        type = type.baseType();

        TypeForm form = type.getForm();

        return type == Predefined.integerType
                ? "IWrap" : type == Predefined.realType
                ? "RWrap" : type == Predefined.booleanType
                ? "BWrap" : form == TypeFormEnumImpl.ENUMERATION
                ? "IWrap" : "CWrap";
    }
}
