...
import com.github.xianzhan.sudoku.enumerable.CStatus;
...
public class SCell extends Button {
    ...
    public void setInitStatus() {
        setEffect(null);
        setStyle("-fx-text-fill: rgb(132, 117, 255)");
        status = CStatus.INIT;
        isHighLight = false;
    }
    ...
    public void setNoteStatus(int fontSize) {
        setStyle(String.format("-fx-text-fill: black; -fx-font-weight: "
                               + "normal; -fx-font-size: %d", fontSize));
        status = CStatus.NOTE;
    }
    ...
    public void setNoteText(String input) {
        ...
        if (!status.equals(CStatus.INIT) || cellText.isEmpty()) {
            if (cellText.contains(input)) {
                cellText = String.join("", cellText.split(input));
            } else {
                cellText += input;
            }
            if (cellText.length() <= 2) {
                fontSize = 24 - cellText.length() * 4;
            } else {
                fontSize = 12;
            }
