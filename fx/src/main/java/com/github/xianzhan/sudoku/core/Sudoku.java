package com.github.xianzhan.sudoku.core;
...
public class Sudoku {
    ...
    private boolean testBlankNumber(Matrix puzzle, int level) {
        ...
        for (int[] row : puzzle.toArray()) {
            ...
            if (rowBlankNumber < minBlankNumber || rowBlankNumber > maxBlankNumber)
                return false;
        }
        ...
        for (int k = 0; k < 9; k++) {
            if (colsBlankNumber[k] < minBlankNumber || colsBlankNumber[k] > maxBlankNumber)
                return false;
            if (blocksBlankNumber[k] < minBlankNumber || blocksBlankNumber[k] > maxBlankNumber)
                return false;
        }
        return true;
    }
